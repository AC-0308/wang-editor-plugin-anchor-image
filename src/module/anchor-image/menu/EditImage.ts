...
function genDomID(): string {
  return genRandomStr('w-e-edit-anchor-image')
}
...
class EditImage implements IModalMenu {
  ...
  private getImageNode(editor: IDomEditor): Node | null {
    return DomEditor.getSelectedNodeByType(editor, 'anchor-image')
  }
  ...
  getModalContentElem(editor: IDomEditor): DOMElement {
    ...
    const [srcContainerElem, inputSrcElem] = genModalInputElems('图片链接', srcInputId)
    ...
    const [hrefContainerElem, inputHrefElem] = genModalInputElems('超链接', hrefInputId)
    const $inputHref = $(inputHrefElem)
    const [buttonContainerElem] = genModalButtonElems(buttonId, '确认')
  ...
  private updateImage(
    ...
    // 修改图片信息
    return updateImageNode(editor, src, alt, href, style)
  }
}
